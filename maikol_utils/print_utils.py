...
def print_log(
    ...
    if logger: # Make sense bc get_logger may return None
        ...
        print(text, file=out_file, end=end)
    else:
        print(text, end=end)
...
def print_separator(text: str = None, sep_type: SepType = "NORMAL", color: Colors = None, out_file: TextIO = None) -> None:
    ...
    if not sep:
        print_warn("WARNING: No separator with that label", out_file=out_file)

    if sep_type == "SUPER":
        print_color(sep, color=color, out_file=out_file)
        if text:
            print_color(f"{text:^{len(sep)}}", color=color, out_file=out_file)
        print_color(sep + "\n", color=color, out_file=out_file)
    elif sep_type == "START":
...
def print_color(text: str, color: Colors = "reset", log_level: LogLevel = 'info', print_text: bool = True, out_file: TextIO = None) -> str:
    ...
    if print_text:
        print_log(f"{text}", log_level=log_level, out_file=out_file)

    return text


def print_warn(text: str, color: Colors = "yellow", prefix: str = '', suffix: str = '', out_file: TextIO = None) -> str:
    ...
    """
    return print_color(f"{prefix}⚠️{text}⚠️{suffix}", color=color, log_level="warning", out_file=out_file)

def print_error(text: str, color: Colors = "red", prefix: str = '', suffix: str = '', out_file: TextIO = None) -> str:
    ...
    """
    return print_color(f"{prefix}❌{text}❌{suffix}", color=color, log_level="error", out_file=out_file)


# ==========================================================================================
#                                    CLEAR LINES
# ==========================================================================================
def print_status(msg: str, log_level: LogLevel = None, out_file: TextIO = None):
    ...
    Args:
        ...
        log_level (Optional[LogLevel], optional): Log level to use. Defaults to None.
    ...
    print_log(f"{clear_line}\r{msg}\r", end="\r", log_level=log_level, out_file=out_file)

def clear_status(log_level: LogLevel = None, out_file: TextIO = None):
    ...
    if log_level is None:
        log_level = get_base_log_level()
    print_status("", log_level=log_level, out_file=out_file)

def clear_bash(n_lines: int = 1, out_file: TextIO = None) -> None:
    ...
    Args:
        ...
        out_file (Optional[TextIO], optional): To print the line somewere that's not the std bash (keed None for bash). Defaults to None.
    """
    print_log("\033[F\033[K"*n_lines, end="", out_file=out_file)  # Move cursor up one line and clear that line

def print_clear_bash(text: str, n_lines: int = 1, log_level: LogLevel = None, out_file: TextIO = None) -> None:
    """Cleans the bash output by removing the last n lines.
    ...
    if log_level is None:
        log_level = get_base_log_level()
    clear_bash(n_lines, out_file=out_file)
    print_log(text, log_level=log_level, out_file=out_file)


def print_utf_8(text: str, print_text: bool = True, out_file: TextIO = None) -> str:
    ...
    if print_text:
        print_log(text, out_file=out_file)
    return text
